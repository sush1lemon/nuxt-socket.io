...
import { a as createNitro, b as build, i as compressPublicAssets, j as writeFile } from './nitro.a440ea11.mjs';
...
async function prerender(nitro) {
  ...
  const generateRoute = async (route) => {
    ...
    const isImplicitHTML = !route.endsWith(".html") && contentType.includes("html");
...
function extractLinks(html, from, res, crawlLinks) {
  ...
  if (crawlLinks) {
    _links.push(
      ...[...html.matchAll(LINK_REGEX)].map((m) => escapeHtml(m[1])).filter((link) => allowedExtensions.has(getExtension(link)))
    );
  }
